...
Most of these example assumes you have a Ray cluster running on Kubernetes. See {ref}`RayCluster Quickstart <kuberay-raycluster-quickstart>` if you don't have a Ray cluster running on Kubernetes.
...
Users can also Ray logs with Rayjobs and RayServices
...
Users can submit a RayJob without specifying a yaml, one can be generated based on the following:
...
| head cpu | 2 |
| head memory | 4Gi |
| worker group name | default group |
| worker replicas | 1 |
| worker cpu | 2 |
| worker memory | 4Gi |
...
#### Submit a Ray Job with a specified rayjob yaml

Users can also designate a specific RayJob yaml

We can add the following yaml file `ray-job.interactivemode.yaml`
...
Note that in the RayJob spec, `submissionMode` must be set to `InteractiveMode`.

...
Accelerator version - accelerator version only. e.g. v6e, v5p, v5litepod
    ...
    def _accelerator_version_check(accelerator_version: str):
        if not accelerator_version in VALID_TPU_TYPES:
            raise ValueError(
    ...
    def _validate_tpu_config(self):
        ...
        if not TPUAcceleratorManager.is_valid_tpu_accelerator_topology(self.accelerator_version):
            raise ValueError("Invalid accelerator topology")

        self._chips_per_host = get_chips_per_host(self._topology, self.accelerator_version)
    ...
    def _reserve_slice(
        ...
        for _ in range(self.num_slices):
            ...
            bundle_label_selector.append({ray._raylet.RAY_NODE_TPU_SLICE_NAME_KEY: slice_name})
...
**kwargs: Additional arguments for the placement group, such as 'name',
    ...
    return SlicePlacementGroup(topology=topology, accelerator_version=accelerator_version, **kwargs)
...
**kwargs: Additional arguments for the placement group, such as 'name',
    ...
    return SlicePlacementGroup(num_slices=num_slices, topology=topology, accelerator_version=accelerator_version, **kwargs)